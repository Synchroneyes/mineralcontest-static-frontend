    ...
    <v-row v-for="(row, rowIndex) in chunkedMaps" :key="rowIndex">
      <v-col v-for="(map, colIndex) in row" :key="colIndex" cols="4">
        <v-card>
          ...
          <v-skeleton-loader
            v-if="loadingMap[`${rowIndex}-${colIndex}`]"
            type="image"
            height="200px"
          />
          <v-img
            v-else
            :src="map.map_thumbnail"
            height="200px"
            @load="onImageLoad(rowIndex, colIndex)"
            @error="onImageError(rowIndex, colIndex)"
          ></v-img>
          ...
          <v-card-subtitle
            class="hover-pointer"
            @click="openDialog(map.map_description)"
          >
            {{ map.map_description }}
          </v-card-subtitle>
